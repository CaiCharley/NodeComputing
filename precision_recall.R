...
ppi_list <- map(files, ~ read_csv(., n_max = 500000) %>% 
    mutate(n = row_number())) %>%
  setNames(files)
...
for (data in by_dataset) {
  ...
  plot <- ggplot(data) +
    ...
    ggtitle(dataname) + 
    ylab("Precision") + xlab("Predicted Interaction #") +
    scale_color_discrete(name = "n Mtoodels")
  ggsave(paste0("~/projects/rrg-ljfoster-ab/caic/princeR/ppis/graphs/",
    dataname, ".png"), plot, device = "png")
}
