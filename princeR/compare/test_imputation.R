myPrInCE <-
  function(
           profiles, gold_standard, impute = F, concat = T, onlyfeat = F, gaussians = NULL, precision = NULL,
...
ggplot(craig[1:1e4, ], aes(1:1e4, precision)) +
  geom_path() +
  geom_path(data = craig_impute[1:1e4, ], color = "red") +
  ggtitle("craig_ifnstim (red = with imputation)")
ggplot(ppis[1:5e4, ], aes(1:5e4, precision)) +
  geom_path() +
  geom_path(data = ppis_imputed[1:5e4, ], color = "red") +
  ggtitle("bn_unstim (red = with imputation)")

ggplot(both_pairs_MLppis[1:1e4, ], aes(1:1e4, precision)) +
  geom_path() +
  geom_path(data = both_pairs_Rppis[1:1e4, ], color = "red") +
  ggtitle("Precision of ML(black)/R(red). bn_unstim, intersection of feature pairs")

ggplot(NULL, aes(rownames(both_pairs_MLppis), rownames(both_pairs_Rppis))) +
  geom_point()