...
my_predict_ensemble = function (dat, labels, classifier = c("NB", "SVM", "RF", "LR"), 
  ...
  if (classifier == "SVM") {
    dat[, !colnames(dat) %in% node_colnames] <- sapply(dat[, 
      !colnames(dat) %in% node_colnames], scale)
  }
  ...
  pb <- progress_bar$new(format = "running fold :what [:bar] :percent eta: :eta", 
   clear = FALSE, total = total_models, width = 80)
  counter <- 0
  for (i in seq_len(models)) {
    # greg greg greg greg greg greg greg greg
    if (is.null(folds)) {
      folds <- cut(seq_len(nrow(training)), breaks = cv_folds, 
        labels = FALSE)
      folds <- sample(folds)
    }
    # greg greg greg greg greg greg greg greg
